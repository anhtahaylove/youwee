...
- **Tùy chọn nâng cao yt-dlp** - Thêm bộ tùy chọn yt-dlp nâng cao theo danh sách cho phép (impersonate, loại IP, socket timeout, user agent, referer, header tùy chỉnh, thời gian nghỉ, số fragment song song, giới hạn tốc độ throttled, kích thước chunk HTTP, bỏ qua giới hạn vùng, bộ lọc match, sắp xếp format và player client của YouTube) kèm kiểm tra hợp lệ ở backend, trong khi cookie, proxy, output và cơ chế thử lại vẫn do Youwee quản lý.
- **Plugin Store** - Duyệt, cài đặt và cập nhật plugin cộng đồng từ danh mục tuyển chọn ngay trong ứng dụng, có xác minh chữ ký và nhà phát hành, kèm danh mục ngoại tuyến và làm mới danh mục từ xa.
- **Khung hộp thoại hỗ trợ tiếp cận** - Thêm `ModalShell` dùng chung, cung cấp cho các hộp thoại tự dựng ngữ nghĩa `role="dialog"`, tên hỗ trợ tiếp cận, đóng bằng Escape, bẫy tiêu điểm và khôi phục tiêu điểm.

### Sửa lỗi

- **Vượt quyền mạng của plugin** - Chỉ cấp `--allow-net` không giới hạn cho plugin Deno khi quyền mạng vừa được khai báo vừa được người dùng phê duyệt. Trước đây plugin chưa được duyệt vẫn có toàn quyền mạng chỉ nhờ manifest.
- **Hủy tải làm tắt ứng dụng khác** - Chỉ dừng đúng các tiến trình yt-dlp/ffmpeg do Youwee tạo ra, theo dõi bằng PID, thay vì tắt mọi tiến trình `yt-dlp`/`ffmpeg` trên máy (vốn cũng tắt luôn các lượt tải khác của Youwee và công việc dựng video hay terminal của người dùng).
- **Tải trùng khi bấm bắt đầu hai lần** - Bỏ qua lệnh `startDownload()` thứ hai khi một lượt tải đang chạy. Trước đây hai lượt chồng nhau tạo hai nhóm worker độc lập và tải cùng một mục hai lần.
- **Lượt tải đã dừng bị khởi động lại sai** - Gắn ID cho từng lượt chạy để một lượt cũ đang kết thúc ngầm không thể xóa trạng thái của lượt mới bắt đầu sau khi Dừng.
- **Thử lại tranh chấp với hàng đợi** - Cập nhật tham chiếu hàng đợi đồng bộ khi thử lại mục lỗi, thay vì bắt đầu tải sau độ trễ cố định 100 ms vốn có thể chạy trước khi trạng thái thử lại được áp dụng.
- **Bộ đếm tự đóng của hộp thoại FFmpeg** - Chuyển bộ đếm tự đóng khi thành công ra khỏi render vào một effect có dọn dẹp, nên nó không còn kích hoạt lại mỗi lần render hay sau khi hộp thoại đã đóng.
- **Xóa nhầm hàng đợi** - Yêu cầu xác nhận trước khi xóa hàng đợi tải, universal, gallery và xuất assets, kèm số mục sẽ bị xóa.
- **Tìm kiếm cài đặt im lặng** - Hiện thông báo "không có cài đặt phù hợp" thay vì đóng popup, và khai báo ô tìm kiếm đúng chuẩn combobox/listbox.
- **Theme picker chưa dịch** - Dịch nút và tiêu đề theme picker (trước đây hardcode tiếng Anh ở mọi ngôn ngữ) và thông báo theme đang chọn cho trình đọc màn hình.
- **Toast không được thông báo** - Đánh dấu vùng toast là live region, dùng `role="alert"` cho toast lỗi, và tạm dừng tự đóng khi focus bằng bàn phím.
- **Viền focus bàn phím** - Thêm viền `:focus-visible` toàn cục cho nút, liên kết và ô nhập để thấy rõ vị trí khi dùng bàn phím. Trước đây 314 control tự style trong 74 file hoàn toàn không có chỉ báo focus.
- **Tải trùng từ liên kết ngoài** - Giải phóng handle `onOpenUrl` khi nó resolve sau lúc effect đã cleanup, tránh listener cũ tải trùng một video.
- **Dấu phân cách tham số extractor của YouTube** - Nối nhiều giá trị `--extractor-args youtube:` bằng `;` thay vì `,`, để khi bật đồng thời phiên bản player.js thực tế và preset player client thì yt-dlp không còn hiểu `player_js_version` là player client thứ hai rồi âm thầm bỏ qua thiết lập.
- **Che thông tin nhạy cảm trong log** - Che giá trị `--user-agent`, `--referer` và `--add-headers` do người dùng nhập trong log lệnh, vẫn giữ tên header để tiện gỡ lỗi.
- **Deep link trên Linux** - Sửa desktop entry trong gói Debian để liên kết `youwee://` đăng ký đúng trên các bản cài Linux.
- **Rò rỉ listener tiến trình phụ thuộc** - Giải phóng listener tiến trình tải FFmpeg và Deno ngay cả khi component bị unmount trước lúc đăng ký hoàn tất, đồng thời xóa các timer hoàn tất còn treo khi unmount.
- **Hộp thoại không có nhãn cho trình đọc màn hình** - Trước đây các hộp thoại cookie, FFmpeg và Deno chỉ là `div` phủ thường, không có role, không có tên hỗ trợ tiếp cận, không bẫy bàn phím và nút đóng không nhãn.

### Thay đổi

- **Phiên bản Rust tối thiểu** - Nâng MSRV khai báo lên 1.82 đúng như mã nguồn vốn đã yêu cầu, loại bỏ toàn bộ cảnh báo MSRV cũ trong `cargo clippy`.
...
- **Đồng bộ nền version custom** - Căn nhánh phát hành custom theo upstream Youwee v0.20.0 nhưng vẫn giữ kênh updater custom và toàn bộ patch custom hiện có
