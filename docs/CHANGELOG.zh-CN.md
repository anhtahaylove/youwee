# 更新日志

Youwee 的所有重要更改都将记录在此文件中。

格式基于 [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)，
本项目遵循 [语义化版本](https://semver.org/spec/v2.0.0.html)。

## [Unreleased]

## [0.20.1-custom.7] - 2026-09-08

### 新增
- **yt-dlp 安装说明** - 当来源固定为“系统”且缺少 yt-dlp 时，显示当前平台的安装方法，而不是仅提示未找到。

### 修复
- **插件权限** - 对于清单未声明网络访问的插件，直接丢弃网络授权而不再存储。运行时本就要求同时具备声明与授权，因此这从来不是权限提升问题；现在存储的状态与实际生效的一致。

## [0.20.1-custom.6] - 2026-09-08

### 新增
- **应用内安装 gallery-dl** - 可直接在 Youwee 中下载 gallery-dl，并按上游校验和进行 SHA-256 验证，无需手动安装。
- **gallery-dl 安装指引** - 未检测到 gallery-dl 时显示当前平台的安装方法，而不是仅提示未找到。
- **插件最低应用版本校验** - 当插件声明的 `minAppVersion` 高于当前应用版本时拒绝下载，而不是安装后在运行时报错。

### 修复
- **FFmpeg 安装指引** - 当来源固定为系统副本且未找到时显示安装方法，此时不提供下载按钮。
- **gallery-dl 更新策略文案** - 不再指向仅存在于 Windows 完整版中的随附副本。
- **Tailwind easing 类歧义** - 用具名的 `ease-out-expo` 工具类替换任意 cubic-bezier 值，消除构建警告。
- **字幕与进度解析性能** - 正则表达式只编译一次，不再为每一行输出重新构建。
- **Windows 上开发服务器崩溃** - 不再让 Vite 监视 Rust 构建目录，此前在 cargo 链接时会因 `app_lib.dll` 的 `EBUSY` 错误导致 `tauri dev` 崩溃。

## [0.20.1-custom.5] - 2026-09-08

### 新增

- **yt-dlp 高级选项** - 新增一组经过允许列表校验的 yt-dlp 高级选项（浏览器模拟、IP 协议族、套接字超时、User-Agent、Referer、自定义请求头、请求间隔、并发分片、限速阈值、HTTP 分块大小、地区限制绕过、匹配过滤器、格式排序以及 YouTube player client），并在后端进行校验；Cookie、代理、输出与重试仍由 Youwee 统一管理。
- **插件商店** - 在应用内的精选目录中浏览、安装和更新社区插件，支持签名与发布者验证，内置离线目录并可远程刷新目录。
- **无障碍对话框外壳** - 新增共享的 `ModalShell`，为独立对话框提供 `role="dialog"` 语义、无障碍名称、Escape 关闭、焦点陷阱与焦点恢复。

### 修复

- **插件网络权限绕过** - 仅当网络权限同时被声明且被用户批准时，才向 Deno 插件授予不受限的 `--allow-net`。此前未经批准的插件仅凭清单即可获得完整网络访问权限。
- **取消下载会终止无关应用** - 仅停止 Youwee 自己启动并按 PID 跟踪的 yt-dlp/ffmpeg 进程，而不是终止机器上所有 `yt-dlp`/`ffmpeg` 进程（这也会终止其他 Youwee 下载以及用户自己的视频剪辑或终端任务）。
- **重复启动导致重复下载** - 当某次运行正在进行时忽略第二次 `startDownload()` 调用。此前两次重叠运行会创建各自独立的工作池，并将同一项目下载两次。
- **已停止的下载被错误重启** - 为每次队列运行标记运行 ID，使后台收尾的旧运行无法清除在“停止”之后启动的新运行的状态。
- **重试与队列竞争** - 重试失败项目时同步更新队列引用，而不是在固定的 100 毫秒延迟后开始下载，因为该延迟可能早于重试状态生效。
- **FFmpeg 对话框自动关闭计时器** - 将成功后的自动关闭计时器从渲染过程移入带清理的 effect，因此它不再在每次重新渲染时重复触发，也不会在对话框关闭后触发。
- **误清空队列** - 清空下载、通用、图库和资源导出队列前需要确认，并显示将丢弃的项目数量。
- **设置搜索无反馈** - 无匹配时显示“没有匹配的设置”而不是直接关闭弹层，并为搜索框补全 combobox/listbox 语义。
- **主题选择器未翻译** - 翻译主题选择器的按钮与标题（此前在所有语言下均为硬编码英文），并向辅助技术公开当前选中的主题。
- **通知未被朗读** - 将 toast 容器标记为实时区域，错误通知使用 `role="alert"`，键盘聚焦时也暂停自动关闭。
- **键盘焦点指示** - 新增全局 `:focus-visible` 焦点环，使按钮、链接和输入框在键盘导航时清晰可见。此前 74 个文件中 314 个自定义控件完全没有焦点指示。
- **外部链接重复下载** - 当 `onOpenUrl` 在 effect 清理后才 resolve 时释放其句柄，避免旧监听器重复触发同一下载。
- **YouTube extractor 参数分隔符** - 多个 `--extractor-args youtube:` 值改用 `;` 连接而非 `,`，避免同时启用实际 player.js 版本与 player client 预设时，yt-dlp 把 `player_js_version` 误当作第二个 player client 而静默丢弃该设置。
- **日志中的敏感高级参数** - 对用户填写的 `--user-agent`、`--referer` 和 `--add-headers` 值进行脱敏，同时保留请求头名称以便排查问题。
- **Linux 深层链接桌面项** - 修复 Debian 安装包的 desktop entry，使 `youwee://` 深层链接在 Linux 上正确注册。
- **依赖进度监听器泄漏** - 即使组件在注册完成前卸载，也会释放启动时注册的 FFmpeg 与 Deno 下载进度监听器，并在卸载时清除其待处理的完成计时器。
- **对话框缺少无障碍标签** - 此前 cookie、FFmpeg 与 Deno 对话框只是普通的覆盖层 `div`，没有 role、没有无障碍名称、没有键盘陷阱，关闭按钮也没有标签。

### 变更

- **最低支持的 Rust 版本** - 将声明的 MSRV 提升到 1.82（代码实际早已需要），清除 `cargo clippy` 中所有过时的 MSRV 警告。

## [0.20.1-custom.4] - 2026-08-15

### 新增

- **H.264 兼容输出** - 为 YouTube 和通用下载新增明确的 Compatible H.264 预设，将所选源转换为采用 H.264 视频、AAC 音频、保留源帧率且像素格式兼容性更广的 MP4，同时继续以 Original (Best) 作为默认最高画质路径。

### 修复

- **兼容输出历史路径** - 在队列和媒体库中记录实际生成的 `.mp4` 兼容文件，避免复用无关的现有文件，并为缩略图和文件元数据保留最终转换后的路径。

## [0.20.1-custom.3] - 2026-08-13

### 修复

- **Facebook Reel 最佳画质** - 为 Best MP4 下载探测 Facebook 直连高清流，仅在达到全高清且至少 50 FPS 时选用，同时保留现有回退以及用户指定的编解码器或帧率限制。

## [0.20.1-custom.2] - 2026-08-13

### 新增

- **可靠性回归测试套件** - 覆盖签名更新清单、持久化队列身份合并、打包的依赖和浏览器扩展资源、Facebook 预览回退、TikTok Live 格式元数据以及 Telegram 控制命令。

### 修复

- **私密运行日志** - 隐藏 Cookie 文件、浏览器配置路径、代理凭据和签名媒体 URL 参数，同时不改变传给 yt-dlp 的实际参数。
- **队列恢复操作** - 所有失败的 YouTube、通用下载和图库项目均可打开日志，非活动图库项目也可复制源 URL。
- **恢复元数据稳定性** - 恢复更新没有新缩略图时保留媒体库中的现有图片，并为 TikTok Live `uhd_60` / `hd_60` 变体识别 60 FPS。

### 变更

- **发布验证** - 在 CI 中运行完整 Bun 回归测试，并在发布更新元数据前验证生成的 `latest.json`。
- **扩展控件** - 弹窗下载选项改用原生 fieldset 语义，同时保留已批准的 AMO 标识和独立扩展版本。

## [0.20.1-custom.1] - 2026-08-11

### 新增

- **文件名元数据前缀** - 可选地在下载文件名前添加上传日期、上传者、观看次数、时长、分辨率和视频 ID，同时保留队列、播放列表和章节编号。
- **小红书图片合集** - 识别仅含图片的小红书帖子，将原图下载到独立文件夹，并保持队列、重复检测和媒体库历史记录一致。

### 修复

- **历史数值安全** - 拒绝 SQLite 中负数的文件大小和时长，并在保存前安全限制过大的无符号数值。

## [0.20.0-custom.53] - 2026-08-02

### 修复

- **客户端 ID 兼容性** - 为不支持 `crypto.randomUUID` 的 WebView 添加安全回退，确保添加和队列操作正常工作。

## [0.20.0-custom.52] - 2026-07-30

### 新增

- **Facebook Story 链接** - 在获取元数据和下载前，将需要认证的 Facebook Story URL 解析为源 Reel 或视频，并复用 Firefox 配置文件 Cookie，无需关闭浏览器。

### 修复

- **确定性输出恢复** - 仅恢复当前下载任务创建或修改的媒体文件，保留分章节输出，并防止将无关文件关联到媒体库历史记录。

## [0.20.0-custom.51] - 2026-07-26

### 修复

- **Facebook Reel 元数据预览** - 当提取器未返回缩略图时，从已选择的 Facebook 视频流生成并缓存首帧预览，同时保留恢复后的标题且不暴露签名流地址。

## [0.20.0-custom.50] - 2026-07-26

### 新增

- **队列 URL 操作** - 可直接从 YouTube 和通用下载队列项目复制源 URL，并显示本地化确认。

### 修复

- **Facebook Reel 占位元数据** - 当 yt-dlp 仅返回 `Facebook` 时，从已有上传者或媒体 ID 恢复可读标题，无需额外元数据请求，并保持队列、文件名和媒体库一致。

## [0.20.0-custom.49] - 2026-07-26

### 修复

- **Facebook Reel 浏览器认证** - 恢复已重命名的 Firefox 默认配置文件，并仅对真实的 Facebook Reel URL 使用浏览器模拟，以提高认证下载的可靠性。
- **Deno Runtime 更新** - 支持 Deno 当前的 PowerShell 风格校验和响应，同时继续强制执行 SHA-256 验证。

## [0.20.0-custom.48] - 2026-07-22

### 新增

- **内置 Firefox 备用包** - 在 Windows 完整安装包中包含 AMO 签名的 Firefox XPI，用于离线恢复，同时继续推荐 Mozilla Add-ons 自动更新渠道。

## [0.20.0-custom.47] - 2026-07-22

### 变更

- **推荐的 Firefox 安装方式** - 在设置中将 Mozilla Add-ons 设为 Firefox 的主要安装入口，并将已签名 XPI 保留在“高级 / 离线安装”中。

### 修复

- **Firefox XPI 备用下载** - 改为直接从 AMO 下载已签名 XPI，避免访问已移除且返回 404 的 GitHub Release 资源。

## [0.20.0-custom.46] - 2026-07-21

### 变更

- **Firefox 稳定渠道** - 将公开 AMO 页面设为正式稳定扩展来源，并为旧的自托管安装保留经过验证的一次性迁移桥接。

## [0.20.0-custom.45] - 2026-07-21

### 修复

- **队列缺失文件同步** - Universal 和 YouTube 队列现在会在队列页面保持打开时自动检测已完成文件被删除的情况，将项目恢复为 Pending，并重新启用 Start Download，无需先进入 Library。

## [0.20.0-custom.1] - 2026-07-20

### 变更

- **自定义版本基线同步** - 将自定义发布线对齐到 upstream Youwee v0.20.0，同时保留自定义更新通道和现有自定义补丁

## [0.19.1-custom.44] - 2026-07-20

### 修复

- **从媒体库删除后的队列恢复** - 下载文件及其历史记录被删除时重新核对已保存的 Universal 和 YouTube 队列项，将其恢复为 Pending，无需再次添加 URL 即可重新下载

## [0.19.1-custom.43] - 2026-07-20

### 修复

- **一致的 Facebook Reel 文件名** - 对 Cookie 模式下的 `Video` 占位标题和未认证模式下带互动前缀的标题统一复用已恢复的元数据标题，使队列、媒体库和下载文件名保持一致

## [0.19.1-custom.42] - 2026-07-19

### 变更

- **更快的 CI 反馈** - 取消同一分支或拉取请求上已过时的 CI 运行，并在 GitHub 托管工作流中使用 checkout v6
- **更安全的自定义发布** - 新增默认仅预览的版本升级工具，验证三个应用版本一致、提升三份更新日志、推导 Windows Installer 版本，并保持扩展商店版本独立

## [0.19.1-custom.41] - 2026-07-18

### 新增

- **托管 Cookie 建议** - 使用 24 小时缓存刷新经过验证的全局 Cookie 跳过目录，同时将个人规则保留在本地并允许独立配置

### 变更

- **更安全的依赖替换** - 将 FFmpeg 与 ffprobe 作为经过验证且可回滚的一组进行更新，并在启用前验证下载的 Deno 二进制文件
- **更保守的 Cookie 默认值** - 在确认某网站持续受益于跳过身份验证前保持全局建议目录为空，同时保留个人规则和旧配置迁移

### 修复

- **Windows 上的 Facebook Reel 文件名** - 根据实际输出目录限制普通标题模板，使长标题或 Unicode 标题的中间视频和音频文件仍可正常写入
- **统一依赖状态** - FFmpeg、Deno 和 gallery-dl 成功完成托管更新检查后显示“已是最新”，而不是回退到随 Youwee 打包的标签

## [0.19.1-custom.39] - 2026-07-18

### 新增

- **应用管理的 gallery-dl 更新** - 运行时检查 gallery-dl 官方发布通道，并在用户明确确认后更新由应用管理的二进制文件
- **清晰的 yt-dlp 通道** - 显示 Bundled、Stable、Nightly 和实验性 Master 通道的已安装及可用版本

### 变更

- **简化 Cookie 设置** - 移除 Quick presets 芯片，同时保留 Facebook Reel 默认跳过 Cookie 规则及用户明确配置

### 修复

- **gallery-dl 已是最新状态** - 当输出确认应用管理的二进制文件已是最新版本时，接受 gallery-dl 的非零退出码

## [0.19.1-custom.38] - 2026-07-18

### 新增

- **面向新手的 Chromium 扩展安装** - Windows 完整安装包内置已解压的 Chromium 扩展，并提供一键打开文件夹、复制安装信息和离线指南

## [0.19.1-custom.37] - 2026-07-17

### 变更

- **扩展版本独立管理** - 从扩展清单读取版本，使桌面应用发布不再与 AMO 版本号冲突

### 修复

- **macOS Gatekeeper 兼容性** - 对 macOS 应用包进行临时签名和验证，让 Apple Silicon 用户可在正式 Developer ID 公证前通过“隐私与安全性”手动打开构建

## [0.19.1-custom.36] - 2026-07-17

### 新增

- **更新传输信息** - 应用更新时显示当前下载速度、预计剩余时间以及安装阶段
- **公开扩展商店资料包** - 添加 AMO 上架包、本地化商店文案、隐私说明以及可直接上传的 Firefox/Chrome 商店图片

### 变更

- **最小浏览器权限** - 移除未使用的 tabs 权限，并将宽泛的 web-accessible 匹配替换为受支持媒体站点白名单

## [0.19.1-custom.35] - 2026-07-17

### 修复

- **更新后日志回退** - 通过 Rust 后端获取发行元数据，使旧版本升级时即使 WebView 因 CORS 无法请求 GitHub 资源也能显示完整发行说明

## [0.19.1-custom.34] - 2026-07-17

### 新增

- **更新后发行说明** - 应用内更新成功并重新启动后，仅显示一次本地化更新日志

### 变更

- **Windows 静默更新** - 按用户安装 Windows 更新，无需再次打开交互式安装界面

### 修复

- **更新器临时存储清理** - 应用启动后从系统临时目录移除残留的 Youwee 更新包

## [0.19.1-custom.33] - 2026-07-17

### 新增

- **公开内容 Cookie 跳过预设** - 添加一键式 YouTube 和 Instagram 芯片，仅追加精确的公开 URL 规则且不替换现有认证配置

## [0.19.1-custom.32] - 2026-07-17

### 变更

- **Firefox Cookie 指引** - 明确说明 Youwee 从所选配置读取 Cookie 时 Firefox 可以保持开启
- **Gallery 认证一致性** - Gallery 下载复用浏览器配置规范化与按 URL 跳过 Cookie 的共享流程

### 修复

- **Universal 队列文件恢复** - 下载媒体文件被删除后将 Completed 项恢复为 Pending，无需重新添加 URL 即可重试
- **队列错误导航** - 让 YouTube、Universal 与 Gallery 失败项目中的 Logs 操作正确打开日志页面以便排查问题

## [0.19.1-custom.31] - 2026-07-17

### 新增

- **Firefox 扩展自动更新** - 让已签名且自行分发的 Firefox 扩展通过经 SHA-256 验证的 GitHub 更新清单发现并安装更高版本的 XPI

### 变更

- **Firefox 发布完整性** - 以自托管模式验证扩展，并要求签名 XPI 与 `firefox-updates.json` 齐全后才发布版本

## [0.19.1-custom.30] - 2026-07-16

### 变更

- **Firefox 数据同意** - 将当前页面 URL 传递声明为必需的浏览活动数据，并要求支持 Mozilla 内置同意流程的 Firefox 版本

### 修复

- **Firefox AMO 验证** - 使用静态标记与原生 DOM API 替换不安全的扩展 `innerHTML` 写入，使打包扩展通过验证且不再产生警告

## [0.19.1-custom.29] - 2026-07-16

### 新增

- **Windows 完整安装包** - 内置经过校验和锁定的 yt-dlp、FFmpeg/ffprobe、Deno 与 gallery-dl，让全新 Windows 安装无需手动配置依赖即可下载

### 变更

- **可移植依赖解析** - 从每台设备的运行时目录解析应用托管、安装包内置和系统二进制文件，并在设置中标识 bundled 工具
- **Windows 发布产物** - 发布带签名的完整 NSIS 与 MSI 产物、匹配的更新元数据以及第三方许可说明

### 修复

- **跨设备依赖路径** - 当应用托管工具缺失时自动使用 Windows 安装包内置二进制，不再依赖构建机器的路径或环境

## [0.19.1-custom.28] - 2026-07-16

### 新增

- **TikTok Live 第一方用户名解析** - 优先从 TikTok 页面解析 room ID，并缓存最近验证的用户名与房间映射，同时不保存签名直播 URL

### 变更

- **Universal 元数据预览** - 使用精简的 UTF-8 yt-dlp 探测，保留远程缩略图来源，并在不阻塞下载的情况下刷新缺失元数据
- **发布产物** - 在发布工作流中准备 Windows MSI 和 Firefox XPI 打包，并在配置 Mozilla 凭据后启用 AMO 签名

### 修复

- **Facebook Reel 跨设备稳定性** - 移除 Reel 分享链接中的追踪参数，让元数据、重复检测、下载与历史记录使用同一稳定 URL
- **yt-dlp 依赖恢复** - 记录所选二进制及执行错误，并在更新后的应用托管可执行文件无法启动时自动重试 bundled 二进制
- **Universal 重复链接反馈** - 在干净 URL 与追踪 URL 间识别已有队列项，聚焦并通知用户，同时重试缺失预览而不是静默重复添加

## [0.19.1-custom.27] - 2026-07-16

### 新增

- **TikTok Live 录制遥测** - 在 Inspect、Record、监控列表和完成结果中显示准确的已录时间、总时长、剩余时间、活动房间容量以及复制/打开 URL 操作

### 变更

- **TikTok Live 预览韧性** - 优先使用 TikTok 动态封面，并在没有可信封面时通过有界、带认证的 FFmpeg 快照生成预览
- **TikTok Live 时长控件** - 使用明确的小时、分钟和秒字段替代原始时长输入，并由共享转换回归测试保障

### 修复

- **TikTok Live Library 缩略图** - 从已完成或恢复的媒体生成持久缩略图，并在 Library 卡片和摘要对话框中一致解析本地缓存图片
- **TikTok Live 用户名查找** - 对签名房间查找的临时 HTTP 失败进行重试，并提示使用数字 room ID，避免把正在直播的流错误报告为离线

## [0.19.1-custom.26] - 2026-07-16

### 新增

- **TikTok Live 预览元数据** - 从 TikTok 房间/页面元数据恢复标题、主播、封面和头像，通过 Youwee 缓存远程图片，并在监控列表中保留最新预览
- **TikTok Live 观众快照** - 在 Inspect 和监控列表中显示主播头像以及本地化的当前或上次观众人数，同时不提高轮询频率

## [0.19.1-custom.25] - 2026-07-15

### 修复

- **Windows 更新器重启提示** - 在下载期间说明 Windows 安装程序会自动关闭并重新打开 Youwee，避免用户等待可能不会出现的“立即重启”按钮

## [0.19.1-custom.24] - 2026-07-15

### 变更

- **TikTok Live 快速录制** - 将目标检查、原始画质/60 FPS/仅音频预设、友好时长、输出目录和主要录制操作放入更清晰的首屏流程，同时保留高级选项

### 修复

- **TikTok Live 自定义规则** - 保持自定义时长、轮询间隔、冷却时间和文件名可编辑，不再因数值恰好匹配预设而自动跳回预设
- **TikTok Live 活跃房间恢复** - 当 yt-dlp 错误报告直播离线时，通过签名房间元数据恢复正在直播的用户名，同时不暴露流 URL，也不向签名服务转发浏览器 Cookie
- **Windows 标题栏最大化** - 让快速双击标题栏与原生最大化/还原状态保持同步，不再立即回到窗口模式

## [0.19.1-custom.23] - 2026-07-15

### 变更

- **TikTok Live Recorder 工作区** - 分离手动录制与监控列表自动化流程，改进直播预览和录制反馈，并完善遥测、响应式布局、空状态和无障碍体验

## [0.19.1-custom.22] - 2026-07-14

### 修复

- **TikTok Live 监控列表日志** - 不再把监控列表轮询中预期的离线状态写入错误日志，同时保留手动 Inspect 错误和真实元数据失败

## [0.19.1-custom.21] - 2026-07-14

### 修复

- **TikTok Live 60 FPS 元数据** - 使用 Web H.265 device type 请求 TikTok 房间元数据，让原始画质在 TikTok 提供时可以暴露 `uhd_60` 1080p60 流

## [0.19.1-custom.20] - 2026-07-13

### 新增

- **TikTok Live 流元数据** - 在 Inspect 结果和 Library 历史记录中更清晰显示 FPS、编码、传输协议、分辨率和码率

### 修复

- **TikTok Live 传输选择** - 当画质相同时优先选择更稳定的 FLV 而不是 HLS，并且仅在元数据明确提供时显示 FPS

## [0.19.1-custom.19] - 2026-07-13

### 修复

- **TikTok Live 原始画质 60 FPS** - 当原始画质提供相同分辨率时优先选择 `uhd_60` 流，让 TikTok 提供时录制保留 60 FPS

## [0.19.1-custom.18] - 2026-07-12

### 修复

- **TikTok Live 原始画质** - 使用 Firefox Cookie 获取 TikTok Live 页面元数据并合并 HEVC `origin`/`uhd_60` 流，让录制在 TikTok 提供时使用 1080x1920 源画质
- **TikTok Live 流选择** - 当画质和分辨率相同，优先选择更稳定的 HLS 而不是低延迟 LLS

## [0.19.1-custom.17] - 2026-07-11

### 新增

- **TikTok Live Telegram 后端命令** - 在 Rust 后端处理 `/tl_*` 监控列表命令，让 Telegram Topic 控制在应用隐藏或最小化时仍可工作

### 修复

## [0.19.1-custom.16] - 2026-07-11

### 新增

- **TikTok Live Recorder Phase 3C-6** - 在重启后保留录制容量限制，新增每位主播的录制时间窗口，显示遥测总览，提示多房间限制，并在签名测试版发布前确认 Telegram Topic 监控列表命令

### 修复

## [0.19.1-custom.15] - 2026-07-11

### 新增

- **TikTok Live Recorder Phase 2A** - 新增有界 FFmpeg 自动重连、带退避的元数据重试、部分录制恢复和详细录制生命周期状态
- **TikTok Live Recorder Phase 2B** - 刷新过期的签名直播 URL，继续写入编号分段，无需转码地合并分段，并在合并失败时保留已录制片段
- **TikTok Live Recorder Phase 2C** - 使用崩溃安全的 Matroska 分段录制，无需转码地重封装为 MP4，并在最终处理失败时保留可播放的 MKV 文件
- **TikTok Live Recorder Phase 2D** - 在 SQLite 中安全持久化录制任务元数据，启动时协调中断会话，并提供继续、完成和确认删除操作，同时不保存签名 URL 或 Cookie 值
- **TikTok Live Recorder Phase 3A** - 新增持久化主播监控列表，支持有界轮询退避、每位主播独立录制规则、离线转直播时自动录制、全局防重复以及重启后的安全状态协调

### 修复

- **TikTok Live 监控列表删除** - 防止后台元数据轮询在通过 Telegram 或界面删除后重新写回已删除的监控项
- **TikTok Live 错误与取消** - 解包嵌套后端 wire error，清晰提示离线直播，并允许在准备元数据时取消

## [0.19.1-custom.14] - 2026-07-10

### 新增

- **TikTok Live Recorder Phase 1** - 新增 TikTok Live 录制页面，支持 Inspect、Record、Cancel、Firefox cookie 认证，并为完成的录制写入 Library/history

### 修复

- **TikTok Live 自动质量** - 优先选择最佳的音视频合流直播流，再回退到仅视频或仅音频格式

## [0.19.1-custom.13] - 2026-07-09

### 变更

- **自定义发布渠道** - 将自定义 updater 元数据、安装包下载和扩展下载链接从 `anhtahaylove/youwee-releases` 迁移到主 fork `anhtahaylove/youwee` 的 release 页面

## [0.19.1-custom.12] - 2026-07-09

### 修复

- **AppImage 图标** - 更新 Tauri 构建工具和兼容依赖，让 Linux AppImage 保留正确的桌面图标元数据

## [0.19.1-custom.11] - 2026-07-08

### 新增

- **日语和西班牙语界面** - 新增日语和西班牙语本地化包，并为自定义版本专属文本保留英文 fallback

## [0.19.1-custom.10] - 2026-07-08

### 新增

- **长视频 AI 摘要** - 将长转录文本拆分为可取消的多段摘要，支持分段/最终摘要输出，并可配置每段字数限制

### 修复

- **AI 摘要有序列表** - 在生成的摘要中保留跨多个 markdown 列表块的有序编号

## [0.19.1-custom.9] - 2026-07-07

### 新增

- **Universal 视频编码** - 允许 Universal Download 选择 H.264、VP9、AV1 或 Auto，并在队列项目中保留所选编码

### 修复

- **OpenAI 摘要参数** - 当模型拒绝某种参数形式时，自动在 `max_tokens` 与 `max_completion_tokens` 之间切换 OpenAI 和代理摘要请求

## [0.19.1-custom.8] - 2026-07-07

### 新增

- **yt-dlp 更新提醒** - 当所选 yt-dlp 渠道有新版本时通知用户，并可从 toast 打开 Dependencies 设置中的更新流程

### 变更

- **摘要阅读字号** - 为新生成和已保存的摘要提供共用的内联字号控制，让阅读更舒适

## [0.19.1-custom.7] - 2026-07-06

### 修复

- **超长 Windows 输出文件夹** - 在非常长或包含 Unicode 的文件夹中保持 yt-dlp 输出路径稳定，并恢复完成后的文件路径，确保 Library/history 正常写入

## [0.19.1-custom.6] - 2026-07-05

### 修复

- **Firefox 认证配置文件** - 在 yt-dlp 读取浏览器 Cookie 前，将 `default-release` 等旧显示名解析为真实的配置文件目录
- **Windows 格式化 churn** - 将仓库文本文件规范为 LF，避免 Biome 和构建过程在 Windows 上重写无关的换行符

## [0.19.1-custom.5] - 2026-07-04

### 修复

- **清除确认** - 对清空 Library 和 Logs 的破坏性操作使用应用内确认对话框
- **Unicode 下载标题** - 优先保留来自 metadata 或最终文件路径的越南语和其他 Unicode 标题，避免被 Windows 进程输出中的乱码标题覆盖

### 变更

- **自定义 fork README** - 为测试用户清晰说明公开 fork 和自定义发布渠道

## [0.19.1-custom.4] - 2026-07-03

### 修复

- **粘贴文本中的 URL** - 在 Download、Universal Download、Gallery 和 Metadata 输入中从普通文本提取有效 URL
- **在文件夹中显示** - 将文件定位操作改为通过应用的 `open_file_location` 命令执行，以提升 Windows 行为一致性
- **History 测试 schema** - 当测试需要新的 metadata 列时，保持 history 数据库测试与旧的内存 schema 兼容

## [0.19.1-custom.3] - 2026-07-03

### 变更

- **发布自动化** - 将自定义源码发布构建保持在仍受维护的 GitHub release action v3
- **扩展发布打包** - 当未配置 AMO 凭据时，允许源码标签构建跳过 Firefox 签名
- **Windows 发布打包** - 源码发布 workflow 仅构建 NSIS，避免自定义 prerelease 版本导致 MSI 打包失败

## [0.19.1-custom.2] - 2026-07-02

### 新增

- **内嵌章节拆分** - 新增可选下载设置，可将视频内嵌章节拆分为独立文件，并可按章节顺序编号

### 变更

- **自动资料库集合** - 启用自动集合时，将拆分出的章节文件也归入资料库集合

## [0.19.1-custom.1] - 2026-07-02

### 新增

- **媒体拆分** - 在资料库中新增手动操作，可将已下载媒体拆分为片段文件，且不会自动产生额外集合
- **自动资料库集合** - 新增可选设置，可将展开的播放列表和频道下载归入资料库集合

### 变更

- **自定义更新版本** - 将自定义版本提升到 `0.19.1-custom.1`，让自定义 updater 可与 `0.19.0-custom.1` 区分

### 修复

- **资料库摘要对话框** - 通过对话框字号控制，让已保存摘要更易阅读

## [0.19.0] - 2026-07-01

### 新增

- **队列项目输出文件夹** - 新增队列项目操作，可为单个下载选择独立下载文件夹
- **队列和播放列表编号** - 新增按队列或播放列表顺序添加文件名前缀的选项
- **重复下载检测** - 在添加下载前根据 Library/history 检测已下载内容，并提供询问、跳过或继续添加选项
- **媒体库删除行为** - 新增设置，可选择仅移除 Library 记录或同时删除本地媒体文件
- **优先 30 FPS 下载** - 为 Download、Universal Download 和 Channels 新增优先 30 FPS 选项
- **清空输入按钮** - 为 Download、Universal Download 和 Gallery 的 URL 输入框新增快速清空按钮

### 变更

- **频道抓取** - 新增停止操作，并在返回刚浏览过的已关注频道时避免不必要的重新抓取
- **队列文件夹变更** - 选择新的全局下载文件夹后，先确认再应用到现有等待队列项目
- **自定义发布链接** - 将 updater、扩展下载链接和应用内 GitHub 链接保持在自定义 `anhtahaylove/youwee` 渠道

### 修复

- **Firefox 认证配置文件** - 将检测到的配置文件传给 yt-dlp 时使用实际的 Firefox 配置目录
- **yt-dlp 匹配过滤器** - 将被 yt-dlp match filter 跳过的下载视为不可重试的 skipped 错误
- **扩展 AI Summary 弹窗** - 优先通过当前标签页的 content script 打开 Summary，并保留直接 deep link 回退
- **扩展冷启动 deep link** - 当 Youwee 从未运行状态被扩展启动时保留待处理下载链接
- **AI Summary 有序列表** - 渲染已保存摘要时保持有序列表编号连续
- **设置图标尺寸** - 让 Font Awesome 图标尺寸与相邻文本保持一致

## [0.18.0] - 2026-06-29

### 新增

- **扩展 AI 摘要** - 在浏览器扩展中新增摘要按钮，可将 YouTube 视频直接打开到 AI Summary
- **AI 摘要 Token 上限** - 在设置中新增可选输入框，用于自定义生成摘要时的最大输出 Token 数
- **Cookie 跳过规则** - 新增按站点匹配的规则，可对匹配 URL 跳过 Cookie 认证，默认包含 Facebook Reels
- **YouTube player client** - 新增 yt-dlp player client 设置，用于处理 YouTube 403 和格式选择问题

### 变更

- **扩展界面** - 优化浏览器扩展弹窗和悬浮菜单，使其更简洁现代，并与音乐播放器风格更一致

### 修复

- **扩展悬浮按钮** - 修复浏览器扩展在已打开的标签页中安装或重新加载后悬浮按钮不显示或崩溃的问题
- **扩展冷启动下载** - 修复桌面应用尚未运行时，在浏览器扩展点击 `Download now` 只打开 Youwee 但未添加视频的问题
- **AI 摘要视频信息** - 修复字幕可用但 yt-dlp 无法选择视频格式时，AI Summary 卡在获取视频信息的问题
- **AI 摘要长度** - 移除默认硬编码的摘要输出 Token 限制，除非用户自定义数值，否则使用模型提供商的默认值

## [0.17.2] - 2026-06-17

### 变更

- **界面字体和图标** - 将应用字体和图标打包进应用，让界面不依赖外部 CDN 也能保持一致显示
- **插件详情布局** - 将设置中已导入插件的详情改为标签页展示，包含信息、请求的权限以及运行时与兼容性
- **频道加载更多** - 将频道浏览页和已关注频道详情页的加载更多按钮改为与 YouTube 关键词搜索一致的浮动样式

### 修复

- **音乐播放器显示状态** - 修复音乐播放器收起后仍留下不可见覆盖层的问题，避免遮挡下方按钮并在点击被遮挡区域时重新开始播放
- **频道完成状态** - 修复重新打开应用后已关注频道视频丢失完成状态的问题，现在会持久化手动频道下载完成状态，并按精确视频 ID 与仍存在的历史文件恢复状态
- **重复关注频道** - 再次关注相同 URL 时返回已有的已关注频道记录，避免视频同步到不存在的频道 ID

## [0.17.1] - 2026-06-11

### 新增

- **即将开始直播的定时下载** - 在队列项目中新增内联操作，可在 YouTube 直播尚未开始时为后续重试设置定时
- **数据导出 YouTube 关键词来源** - 新增 YouTube 关键词来源，可在 Data Export 中导出搜索结果
- **CLI 输出文件夹** - 新增 `--output` / `-o`，让每个通过 CLI 加入队列的下载使用自己的绝对输出文件夹

### 变更

- **定时控件** - 优化定时弹窗和已定时状态，加入更快的预设、更清晰的预览文本和更紧凑的倒计时显示
- **CLI 设置与文档** - 优化 General 设置中的 CLI 卡片，补齐所有支持语言的 CLI 本地化，并扩展 CLI 指南中的 macOS、Windows 和 Linux 安装说明

### 修复

- **应用管理 FFmpeg 更新** - 记录已安装应用管理 FFmpeg 的 release 版本，避免二进制报告内部 git 构建版本时重复提示更新
- **FFmpeg 依赖状态** - 将自动检测到的系统 FFmpeg 显示为 System 而不是 App managed，并提供更清晰的应用管理安装验证错误
- **yt-dlp 内置版本** - 优先使用当前应用构建随附的 sidecar，避免旧的应用管理二进制遮蔽更新的内置版本
- **频道 yt-dlp 来源** - 修复频道浏览未正确使用所选 yt-dlp 来源/通道的问题，包括 Stable 和 System
- **粘贴转义 URL** - 在 Download、Universal 和 Gallery 输入框中规范化被 shell 转义的 URL 标点，使 `watch\?v\=...` 形式的链接仍能加入目标视频
- **CLI 转义 URL** - 规范化被 shell 转义的 URL 标点，使 `watch\?v\=...` 形式的 YouTube URL 仍能下载目标视频
- **已排期直播错误** - 为即将开始的 YouTube 直播显示更明确的提示，不再归入通用的 skipped 或 unspecified 错误
- **Windows CLI 输出** - 修复 `youwee -V` 和 `youwee --help` 在 Windows 终端中不输出内容的问题

## [0.17.0] - 2026-06-07

### 新增

- **命令行界面** - 新增本地 `youwee` CLI，包含安装入口、结构化下载请求，并支持质量、音频模式、仅加入队列、播放列表、字幕、下载片段、从头下载直播和跳过直播等选项
- **跳过直播** - 在 YouTube 和 Universal 界面新增下载设置，可跳过当前正在直播的视频
- **AI 摘要搜索** - 新增基于 SQLite FTS5 的媒体库搜索，可检索标题、URL、文件路径和已保存的 AI 摘要，并提供全部内容、仅详情或仅 AI 摘要的搜索范围控制
- **YouTube 关键词搜索** - 新增独立的 YouTube 关键词搜索视图，支持按上传日期、时长、排序方式和视频特性进行筛选，并可将选中的结果直接加入下载队列
- **队列状态标签页** - 为 YouTube 和 Universal 队列新增紧凑的状态标签页，可按下载状态筛选视频
- **Plugin SDK YouTube 搜索桥接** - 通过 `ctx.youwee.youtube.searchVideos(...)` 向 JavaScript 插件开放由应用托管的 YouTube 关键词搜索，并提供强类型筛选和 continuation 支持

### 变更

- **AI 生成超时** - 将 AI 生成超时选项扩展到最长 60 分钟，并将所选超时应用到各 AI 提供商的 HTTP 请求，以支持长视频摘要生成
- **应用数据库文件名** - 将内部 SQLite 数据库从 `logs.db` 重命名为 `youwee.db`，并自动从旧文件迁移且保留旧文件作为备份

### 修复

- **Bilibili HTTP 412** - 为 Bilibili yt-dlp 请求添加类似浏览器的 header，避免 `HTTP Error 412`

## [0.16.0] - 2026-06-02

### 新增

- **持久化下载队列** - 新增下载设置选项，可将 YouTube、Universal 和 Gallery 队列项目保存到应用数据库中，以便关闭并重新打开 Youwee 后恢复队列
- **数据导出** - 新增数据导出工作区，可从 YouTube 播放列表和频道导出列表，自由选择要包含的字段，保存为 CSV、Excel、JSON、Markdown、HTML、SQLite、Word 等格式，并将导出的文件保存到媒体库，方便之后再次打开
- **Telegram 远程下载** - 新增 Remote Download 设置区域，支持通过 Telegram long polling 进行控制、以 tag 形式输入允许的 chat ID、命令指南弹窗、`/add`、`/download`、`/status`、`/queue`、`/stop`、`/help` 命令，以及 `720`、`audio`、`mp3` 等简短质量参数

### 变更

- **下载时继续添加队列** - 允许在下载进行中继续向 YouTube、Universal 和 Gallery 队列添加新 URL，并让队列 worker 在结束当前下载会话前短暂等待新加入的项目
- **YouTube 格式选择** - 将 YouTube 默认视频编码改为 Auto，使新的下载不再强制使用 H.264-only 格式选择，并在视频没有匹配 AVC 流时更接近 Universal 下载行为

### 修复

- **Linux deb 安装冲突** - 将内置 yt-dlp sidecar 重命名为 Youwee 专用二进制名称，使 `.deb` 安装不再与发行版管理的 `yt-dlp` 包冲突
- **系统依赖路径** - 改进 Windows 上 yt-dlp、FFmpeg、Deno、gallery-dl 及辅助工具的 PATH 解析
- **Firefox Cookie 配置文件选择** - 优先使用 `profiles.ini` 中的活动 Firefox 配置文件，让浏览器 Cookie 下载更可能选中实际保存 Cookie 的配置文件

## [0.15.1] - 2026-05-27

### 变更

- **UI 与 UX 优化** - 打磨了 AI Features、metadata、插件设置、guide 弹窗以及共享通知流程的界面与体验，让 Youwee 整体风格更加一致
- **支持 Plugin SDK v2.0.0** - 将插件能力升级到 `youwee-sdk` `v2.0.0`，包含更严格的 `read/write/AI` 权限约束，以及 TypeScript 优先的插件工作区支持

## [0.14.1] - 2026-05-24

### 变更

- **插件引导与权限流程** - 优化了插件导入、权限审批、workspace 指南和配置界面，使插件安装、工作流分配以及在 Settings 中阅读 Guide 更清晰
- **提升插件作者灵活性** - 更新 workspace 与 SDK 集成，支持更灵活的 Lucide 插件图标，并为插件作者提供更简洁的 Deno 本地测试流程

### 修复

- **插件运行日志与输出摘要** - 减少重复的插件日志噪音，不再保存插件结果的原始协议输出，并在 post-processing 日志中更清楚地显示后续步骤会使用的输出文件路径
- **插件 Guide 与多语言文档显示** - 修复 Guide 弹窗内容溢出的问题，并保留已安装插件的 `README.<locale>.md` 文件
- **插件配置界面打磨** - 改进 multi-select 控件、校验触发时机，以及导入/启用插件时的提示，使其与应用整体界面更一致

## [0.14.0] - 2026-05-24

### 新增

- **媒体库标签与合集** - 为媒体库项目新增自由标签和虚拟合集，包含逐项分配、快捷 chip 筛选、合集管理，以及按标签或合集进行高级筛选
- **已签名插件系统与 SDK 工作流** - 新增已签名的 `.ywp` 插件系统，包含 workspace 附加/调试流程、强类型配置字段、多语言插件指南、权限审批、工作流分配、日志查看，以及通过 `youwee-sdk` 进行打包与签名

### 变更

- **Gallery URL 输入与 YouTube 对齐** - 更新 Gallery 输入区域，使其更接近 YouTube 的 single/multiple 流程，包括 Add 按钮样式、批量布局、URL 提示以及多语言文案

### 修复

- **Arch Linux 启动时的 GBM EGL display 崩溃** - 为 Linux 上的 WebKitGTK 添加后备处理，默认禁用 GBM renderer，以避免部分 Arch 系发行版在启动时直接中止

## [0.13.3] - 2026-05-14

### 新增

### 变更

### 修复

- **已关注频道的视频加载循环** - 修复一个由频道分页引入的回归问题，该问题会让已关注频道卡在持续获取状态，无法稳定拉取视频列表
- **频道获取进度竞态** - 频道浏览进度现在会忽略来自旧请求的过期事件，避免用户快速切换频道或重复刷新时显示错乱
- **加载更多导致新视频计数膨胀** - 从频道后续分页加载的旧视频不再被保存为新项目，避免新视频徽标和托盘计数错误增加

## [0.13.2] - 2026-05-10

### 新增

- **频道视频分页与“加载更多”** - 频道页面现在默认加载前 100 个视频，并可在浏览视图和详情视图中按批继续加载更多内容
- **泰语支持** - 为整个应用新增完整泰语本地化，覆盖界面页面、设置、字幕工具、下载流程和语言选择器
- **阿拉伯语支持** - 为整个应用新增完整阿拉伯语本地化，覆盖界面页面、设置、字幕工具、下载流程，并加入 RTL 文档方向处理

### 变更

### 修复

- **频道浏览被限制为 50 个视频** - 修复频道和播放列表浏览在仍有更多内容时错误地停在 50 个视频的问题
- **Cookie 错误弹窗分类不正确** - 修复“需要最新登录 Cookie”被误显示为浏览器 Cookie 数据库锁定的问题；现在会分别显示数据库锁定与认证 Cookie 需求的不同弹窗

## [0.13.1] - 2026-04-26

### 新增

- **LM Studio AI 提供商** - 新增 LM Studio 作为本地 OpenAI 兼容 AI 提供商，支持自定义本地端点且无需 API 密钥

### 变更

### 修复

- **4K WebM 后处理** - 修复 WebM 下载可能选到 MP4/H.264 兼容流，导致 FFmpeg 在后处理转换阶段失败的问题

## [0.13.0] - 2026-04-15

### 新增

- **悬浮音乐播放器** - 新增应用内音频播放器，支持播放队列、播放控制、倍速与音量调节
- **集成 Aria2 外部下载器** - 新增 `aria2c` 外部下载支持，提供可自定义参数与本地化错误处理
- **支持在队列与媒体库中重命名已下载文件** - 新增下载完成后的重命名操作（YouTube/Universal 队列与媒体库），并同步数据库路径/标题与多语言文案
- **媒体库高级筛选与排序** - 新增 Advanced Filters 面板（媒体类型、日期范围、格式、质量）、`title + filepath` 搜索，以及可记忆排序偏好的历史列表排序
- **独立的 Gallery 下载页面** - 在 Universal 下方新增 `Gallery` 菜单，提供独立的 URL 输入、批量导入、队列、输出目录选择，以及基于 `gallery-dl` 的 Start/Stop 下载流程

### 变更

- **下载进行中的动态队列处理** - 队列 worker 改为运行时领取任务，新添加的视频会追加到队尾并自动继续下载，无需再次点击 Start

### 修复

- **CI 上的历史重命名单元测试** - 为共享 in-memory 数据库的测试增加串行保护，避免并行 `cargo test` 下出现 `History entry not found` 的随机失败
## [0.12.0] - 2026-03-04

### 新增

- **依赖来源切换（yt-dlp/FFmpeg）** - 在 设置 → 依赖 中新增来源选择，可在“应用管理”和“系统管理”之间切换
- **切换到系统来源前的安全确认** - 切换 yt-dlp/FFmpeg 到系统来源时新增确认弹窗，避免误操作

### 变更

- **系统来源标签按操作系统显示** - 系统来源标签会按平台显示（macOS 显示 `Homebrew`，Windows 显示 `PATH`，Linux 显示包管理器）
- **构建流程自动生成 GitHub 发布说明** - 在发布 workflow 中启用 `generate_release_notes`，发布时可自动附带生成的版本说明
- **Windows 自定义标题栏集成** - 用应用主题风格的自定义窗口控制（拖拽区、最小化/最大化/关闭）替换原生 Windows 标题栏

### 修复

- **Windows 下载历史记录缺失** - 在 Windows 上更可靠地捕获最终输出文件路径，确保下载完成后正确写入媒体库历史
- **Windows 重新下载路径解析** - 修复重下时输出目录提取逻辑，正确处理 `\` 分隔符路径
- **Windows 非 UTF-8 的 yt-dlp 输出** - 增加 GBK/ANSI 解码回退与 `--print-to-file` 处理，确保非 UTF-8 区域设置也能正确获取最终路径
- **下载完成后媒体库自动刷新** - 当下载状态变为 `finished` 时，媒体库历史会自动刷新
- **Douyin modal 链接兼容性** - 将带 `modal_id` 的 `douyin.com` 链接规范化为 `/video/{id}`，并应用到后端 yt-dlp 调用和前端 deep-link 解析

## [0.11.1] - 2026-03-01

### 新增

- **法语、葡萄牙语和俄语支持** - 完整的 Français、Português 和 Русский 本地化，覆盖所有界面、设置、错误消息和元数据标签
- **后端错误消息本地化** - 后端错误消息（下载失败、网络错误等）现在按用户所选语言翻译显示，不再始终显示英文

### 变更

- **重构字幕回退链** - 统一 AI 摘要和处理任务中的字幕回退逻辑，使行为更加一致

### 修复

- **Douyin 和 TikTok 的字幕回退** - 改进 Douyin 和 TikTok 视频的字幕提取，此前会静默失败
- **字幕错误与短字幕处理** - 字幕错误现在会保留用于诊断，而非被静默吞掉；短字幕被接受为有效字幕，不再被拒绝
- **TikTok 默认设置** - 调整 TikTok 默认下载设置以符合平台惯例

## [0.11.0] - 2026-02-20

### 新增

- **浏览器扩展一键下载（Chromium + Firefox）** - 现在你可以把浏览器当前视频页面直接发送到 Youwee，并选择 `Download now` 或 `Add to queue`
- **设置中新增 Extension 安装入口** - 在 设置 → Extension 新增专属区域，提供直接下载按钮和简单安装步骤（Chromium / Firefox）

### 变更

- **YouTube 与 Universal 页面 UI/UX 更新** - 统一并简化链接输入、预览卡片、队列与标题栏交互，整体体验更清晰一致

### 修复

- **依赖解析在各功能中保持一致** - 统一 download、metadata、channels 与后台 polling 的 yt-dlp/FFmpeg 来源解析，确保始终遵循已选来源
- **系统模式缺失二进制时明确失败** - 选择系统来源但缺少二进制时，应用会明确报错，不再静默回退

## [0.10.1] - 2026-02-15

### 新增

- **ASS 字体设置** - 新增 ASS 导出与字幕预览的字体/字号配置
- **换行工作流优化** - 新增一键自动换行，并支持编辑时使用 Shift+Enter 插入换行
- **可配置自动重试** - 为 YouTube 和 Universal 下载新增 Auto Retry 设置，可自定义重试次数与间隔，在网络不稳定或直播中断时自动恢复下载

### 变更

### 修复

- **下载失败诊断更清晰** - 优化 yt-dlp 失败信息，返回更明确的错误原因，便于识别临时错误并触发更准确的自动重试

## [0.10.0] - 2026-02-15

### 新增

- **字幕工坊** - 新增一体化字幕页面，支持 SRT/VTT/ASS，包含文本编辑、时间轴工具、查找替换、自动修复和 AI 功能（Whisper、翻译、语法修正）
- **高级字幕工具** - 新增波形/频谱时间轴、镜头切换同步、基于风格预设的实时质检、拆分合并、双栏翻译模式（原文/译文）以及项目批处理工具

### 变更

### 修复

## [0.9.4] - 2026-02-14

### 新增

- **Processing 输出目录选择器** - 在 Processing 聊天区新增输出目录选择按钮。默认仍为主视频所在目录，AI 命令与快速操作都会使用所选目录输出
- **Processing AI 聊天多类型附件** - Processing 聊天现支持图片/视频/字幕附件（文件选择 + 拖拽），并按类型显示预览与元数据
- **设置中的语言投票快捷入口** - 在 设置 → 通用 中新增快捷链接，用户可前往 GitHub Discussions 投票/请求下一种支持语言
- **系统托盘新增应用更新检查** - 在托盘菜单新增动作，可直接检查 Youwee 应用更新

### 变更

- **更稳定的字幕/合并命令生成** - Processing 命令生成在回退到 AI 前，优先走确定性逻辑处理字幕烧录与多视频合并（含 intro/outro 顺序提示）
- **托盘频道检查文案更清晰** - 将 “立即全部检查” 调整为更明确的已关注频道检查文案，避免歧义
- **页面标题更简洁** - 移除 Metadata、Processing、AI Summary 页面标题前的图标，界面更清爽

### 修复

- **启用认证/代理时获取视频信息失败** - 修复 yt-dlp 参数顺序，将 cookie 和代理参数插入到 URL 分隔符 `--` 之前，避免出现 `Failed to fetch video info`，同时保持下载流程正常
- **Stable 频道更新检查始终显示可更新** - 修复 stable/nightly 的 yt-dlp 更新检查逻辑，改为读取已安装频道二进制的真实版本（`--version`），而不是仅依据文件存在元数据，从而在更新后正确显示“已是最新”
- **Bundled 更新状态与实际使用二进制不一致** - 修复 bundled 更新流程：在设置页正确显示最新可用版本，并在存在时优先使用已更新的 `app_data/bin/yt-dlp`，使 bundled 更新真正生效
- **优化 Processing 页面视频信息区** - 将播放器下方信息重做为 YouTube 风格（标题 + 现代化元数据标签），并移除编解码标签的 hover 变色与阴影，视觉更简洁
- **Prompt Templates 下拉框不会自动关闭** - 修复 Processing 的 Prompt Templates 下拉框，现在点击外部区域或按 Escape 键会自动关闭
- **Universal URL 计数重复显示** - 修复 Universal 输入框 URL 计数徽标重复数字的问题（例如 `1 1 URL`）

## [0.9.3] - 2026-02-14

### 新增

- **元数据中下载字幕** - 在元数据设置栏新增字幕开关，可在获取元数据时同时下载字幕（手动 + 自动生成）。包含弹出窗口用于选择语言和格式（SRT/VTT/ASS）

### 变更

- **改进时间范围输入体验** - 将普通文本输入替换为自动格式化时间输入，输入时自动插入 `:` 分隔符（如 `1030` → `10:30`，`10530` → `1:05:30`）。智能占位符根据视频时长显示 `M:SS` 或 `H:MM:SS`。实时验证，格式无效或开始时间 >= 结束时间时显示红色边框。可用时显示视频总时长提示

## [0.9.2] - 2026-02-13

### 新增

- **按时间范围下载** - 通过设置开始和结束时间仅下载视频的特定部分（如 10:30 到 14:30）。可在 YouTube 和 Universal 队列中通过剪刀图标为每个视频单独设置。底层使用 yt-dlp 的 `--download-sections`
- **启动时自动检查 FFmpeg 更新** - FFmpeg 更新检查现在在应用启动时自动运行（仅限内置安装）。如有可用更新，将在设置 > 依赖项中显示，无需手动点击刷新按钮

## [0.9.1] - 2026-02-13

### 修复

- **未安装 Homebrew 的 macOS 上应用崩溃** - 修复因缺少 `liblzma` 动态库导致的启动崩溃。`xz2` crate 现使用静态链接，使应用完全独立，无需 Homebrew 或系统库
- **自动下载忽略用户设置** - 频道自动下载现在使用每个频道的独立下载设置（视频/音频模式、画质、格式、编解码器、比特率），不再使用硬编码默认值。每个频道在频道设置面板中可单独配置下载偏好
- **安全加固** - FFmpeg 命令现使用结构化参数数组代替 shell 字符串解析，防止命令注入。为所有 yt-dlp 调用添加 URL scheme 验证和 `--` 分隔符以阻止选项注入。启用内容安全策略，移除过宽的 shell 权限，并为渲染链接添加 `isSafeUrl` 验证
- **MKV/AVI/FLV/TS 容器视频预览失败** - 预览检测现在同时检查容器格式和编解码器。不受支持的容器（MKV、AVI、FLV、WMV、TS、WebM、OGG）中的视频会自动转码为 H.264 预览。macOS 上 MP4/MOV 中的 HEVC 不再被不必要地转码
- **定时下载在托盘模式下不可见** - 当应用最小化到系统托盘时，定时下载开始、停止或完成时会发送桌面通知。托盘菜单显示活动定时状态（如"YouTube: 23:00"）。定时功能在 YouTube 和 Universal 页面均可使用
- **从托盘退出会中断活动下载** - 托盘"退出"现在使用优雅关闭而非强制终止进程，允许活动下载完成清理，防止文件损坏
- **Dock 图标设置重启后丢失 (macOS)** - "关闭时隐藏 Dock 图标"偏好设置现在在应用启动时同步到原生层，而非仅在访问设置页面时同步
- **通用队列加载时显示骨架屏而非 URL** - 将闪烁的骨架屏占位符替换为实际 URL 文本和"正在加载信息..."旋转徽章。当元数据获取失败时，项目现在会正常退出加载状态，而非永远显示骨架屏

## [0.9.0] - 2026-02-12

### 新增

- **频道关注与自动下载** - 关注 YouTube 频道，浏览视频，批量选择下载，支持完整的画质/编解码器/格式设置。后台轮询检测新上传并发送桌面通知，每频道显示新视频数量徽章。可折叠的关注频道面板，支持系统托盘最小化
- **大文件预览确认** - 可配置的文件大小阈值（默认 300MB），在处理页面加载大型视频前显示确认对话框。可在设置 → 通用 → 处理中调整阈值
- **多语言设置搜索** - 设置搜索现在支持所有语言。用中文搜索（如"主题"）或越南文搜索均可返回匹配结果。英文关键词始终作为后备方案

### 修复

- **Linux 上处理页面加载 4K VP9/AV1/HEVC 视频时白屏** - GStreamer 的 AAC 音频解码器在播放 VP9/AV1/HEVC 视频时导致 WebKitGTK 崩溃。预览现使用双元素方案：静音 H.264 视频 + 独立 WAV 音频通过 JavaScript 同步播放，完全绕过有问题的 AAC 路径。若视频播放仍失败，自动回退到静态 JPEG 缩略图。支持 macOS、Windows 和 Linux

## [0.8.2] - 2026-02-11

### 新增

- **多语言更新说明** - 更新对话框现在按用户语言显示发布说明（英文、越南文、中文）。CI 自动从对应语言的 CHANGELOG 文件中提取更新日志
- **通用下载支持 8K/4K/2K 画质** - 画质下拉菜单新增 8K 超高清、4K 超高清和 2K QHD 选项，与 YouTube 标签页一致。如果来源没有高分辨率格式，会自动降级到最高可用画质
- **通用下载"从头录制直播"开关** - 高级设置中新增开关，可从直播开头录制而非当前时间点。使用 yt-dlp 的 `--live-from-start` 参数
- **通用下载视频预览** - 添加 TikTok、Bilibili、Facebook、Instagram、Twitter 等网站的 URL 时自动显示缩略图、标题、时长和频道。缩略图也会保存到媒体库历史记录
- **更智能的平台识别** - 媒体库现在能正确识别和标记 yt-dlp 支持的 1800 多个网站（Bilibili、Dailymotion、SoundCloud 等），不再显示"其他"。新增 Bilibili 专用筛选标签

### 修复

- **Linux 上处理页面上传视频时冻结** - 视频文件通过 `readFile()` 被完整读入内存，导致内存溢出和白屏。现在使用 Tauri 的 asset 协议直接流式传输视频，无需加载到内存。同时添加错误边界防止白屏、带编解码器信息的视频错误处理、blob URL 清理防止内存泄漏、以及非 MP4 格式的正确 MIME 类型检测
- **媒体库中缩略图损坏** - 修复 Bilibili 等使用 HTTP URL 的网站缩略图。缩略图加载失败时现在会优雅地显示占位图标
- **切换页面时媒体库不刷新** - 媒体库现在在导航到页面时自动加载最新下载记录，无需手动刷新
