...
- **yt-dlp advanced options** - Add an allowlisted set of advanced yt-dlp options (impersonation, IP family, socket timeout, user agent, referer, custom headers, sleep intervals, concurrent fragments, throttled rate, HTTP chunk size, geo bypass, match filters, format sort, and YouTube player client) with server-side validation, so Youwee-managed cookie, proxy, output, and retry handling stays authoritative.
- **Plugin Store** - Browse, install, and update community plugins from a curated in-app catalog, with signature and publisher verification, a bundled offline catalog, and a remote catalog refresh.
- **Accessible dialog shell** - Add a shared `ModalShell` that gives the standalone dialogs proper `role="dialog"` semantics, an accessible name, Escape-to-close, a focus trap, and focus restoration.

### Fixed

- **Plugin network permission bypass** - Grant Deno plugins unrestricted `--allow-net` only when the network permission is both declared *and* approved by the user. Previously an unapproved plugin still received full network access from its manifest alone.
- **Cancelling a download killed unrelated apps** - Stop only the yt-dlp/ffmpeg processes Youwee itself started, tracked by PID, instead of killing every `yt-dlp`/`ffmpeg` process on the machine (which also terminated other Youwee downloads and the user's own video-editing or terminal jobs).
- **Duplicate downloads from double start** - Ignore a second `startDownload()` call while a run is active. Two overlapping runs previously created independent worker pools that claimed and downloaded the same items twice.
- **Stopped downloads restarting incorrectly** - Tag each queue run with an ID so a stale run unwinding in the background can no longer clear the state of a newer run started after Stop.
- **Retry racing the queue** - Update the queue reference synchronously when retrying a failed item instead of starting the download after a fixed 100 ms delay, which could start before the retry was applied.
- **FFmpeg dialog auto-close timer** - Move the success auto-close timer out of render into a cleanup-aware effect, so it no longer re-fires on every re-render or after the dialog closes.
- **Accidental queue wipes** - Require confirmation before clearing the download, universal, gallery, and assets export queues, showing how many items will be discarded.
- **Silent settings search** - Show a "no matching settings" message instead of closing the results popup, and expose the search as a proper combobox with listbox results.
- **Untranslated theme picker** - Translate the theme picker trigger and heading, which were hardcoded English in every locale, and expose the selected theme to assistive technology.
- **Unannounced toasts** - Mark the toast viewport as a live region, announce error toasts via `role="alert"`, and pause auto-dismiss on keyboard focus as well as pointer hover.
- **Keyboard focus indicators** - Add a global `:focus-visible` ring so buttons, links and inputs are visible when navigating by keyboard. Previously 314 hand-styled controls across 74 files had no focus indicator at all.
- **Duplicate downloads from external links** - Release the `onOpenUrl` deep-link handle when it resolves after the effect was already cleaned up, preventing a stale listener from starting the same download twice.
- **YouTube extractor argument separator** - Join multiple `--extractor-args youtube:` values with `;` instead of `,`, so combining the actual player.js version with a player client preset no longer makes yt-dlp read `player_js_version` as a second player client and silently drop the setting.
- **Sensitive advanced arguments in logs** - Redact user-supplied `--user-agent`, `--referer`, and `--add-headers` values in command logs while keeping the header name visible for debugging.
- **Linux deep-link desktop entry** - Repair the Debian package desktop entry so `youwee://` deep links register correctly on Linux installs.
- **Dependency progress listeners leaking** - Release the FFmpeg and Deno download-progress listeners registered during startup even when the component unmounts before registration resolves, and clear their pending completion timers on unmount.
- **Unlabelled dialogs for screen readers** - The cookie, FFmpeg, and Deno dialogs previously rendered as plain overlay `div`s with no role, no accessible name, no keyboard trap, and an unlabelled close button.

### Changed

- **Minimum supported Rust version** - Raise the declared MSRV to 1.82, which the codebase already required, clearing every stale MSRV warning from `cargo clippy`.
...
- **Reliability regression suite** - Cover signed updater manifests, persisted queue identity merging, packaged browser-extension/dependency assets, Facebook preview fallback, TikTok Live format metadata, and Telegram controller commands.

### Fixed

- **Private runtime logs** - Redact cookie files, browser profile paths, proxy credentials, and signed media URL parameters without changing the arguments passed to yt-dlp.
- **Queue recovery actions** - Keep View Logs available for every failed YouTube, Universal, and Gallery item, and allow inactive Gallery items to copy their source URL.
- **Recovered metadata stability** - Preserve an existing Library thumbnail when a recovery update has no replacement and infer 60 FPS for TikTok Live `uhd_60` / `hd_60` variants.

### Changed

- **Release validation** - Run the complete Bun regression suite in CI and validate generated `latest.json` before publishing updater metadata.
- **Extension controls** - Use native fieldset semantics for popup download options while retaining the approved AMO identity and separate extension version.
...
- **Public cookie-skip presets** - Add one-click YouTube and Instagram chips that append narrow public URL patterns without replacing existing authentication rules
...
- **Windows updater restart guidance** - Explain during download that the Windows installer closes and reopens Youwee automatically instead of asking users to wait for a Restart Now button that may never appear
...
- **TikTok Live Recorder Phase 2D** - Persist safe recording job metadata in SQLite, reconcile interrupted sessions at startup, and provide Continue, Finalize, and confirmed Delete recovery actions without storing signed URLs or cookie values
...
- **Release automation** - Keep custom source release builds on the maintained GitHub release action v3
- **Extension release packaging** - Let source tag builds skip Firefox signing when AMO credentials are not configured
- **Windows release packaging** - Build NSIS only in source release workflows so custom prerelease versions do not fail MSI packaging
