...
use crate::database::{add_history_internal, delete_history_from_db};
...
pub async fn split_media_segments(
    ...
    for (index, segment) in request.segments.iter().enumerate() {
        ...
        created_output_paths.push(output_path);
...
mod tests {
    ...
    fn sanitizes_filename_parts() {
        assert_eq!(
            sanitize_filename_part("  <Part: 01?> / intro  ", "fallback"),
            "Part 01 intro"
        );
    ...
    fn rejects_invalid_segments() {
        let invalid = MediaSplitSegmentRequest {
            name: "bad".to_string(),
            start_seconds: 10.0,
            end_seconds: 5.0,
        };
        assert!(validate_segment(&invalid).is_err());

        let valid = MediaSplitSegmentRequest {
            name: "good".to_string(),
            start_seconds: 5.0,
            end_seconds: 10.0,
        };
        assert!(validate_segment(&valid).is_ok());
    }
}
